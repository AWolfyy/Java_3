...
public class ClientHandler {
    ...
    public ClientHandler(Socket socket, ChatServer chatServer, Logger logger) {
        ...
        while (true) {
            ...
            try {
                ...
                if (massage.startsWith("/auth")) {
                    logger.info("Клиент прислал команду /auth");
        ...
        while (true) {
            try {
                ...
                if (message.startsWith("/pm")) {
                    logger.info("Клиент с ником {} прислал команду /pm", nick);
                    sendPmMessage(message);
                } else if (message.startsWith("/history")) {
                    logger.info("Клиент с ником {} прислал команду /history", nick);
                    loadHistory();
                } else if (message.startsWith("/help")) {
                    logger.info("Клиент с ником {} прислал команду /help", nick);
                    loadHelp();
                } else if (message.startsWith("/changeNick")) {
                    logger.info("Клиент с ником {} прислал команду /changeNick", nick);
                    changeNick(message);
                } else if (message.equals("/quit")) {
                    logger.info("Клиент с ником {} прислал команду /quit", nick);
                    closeConnection();
                    break;
                } else {
                    if (!message.isEmpty()) {
    ...
    private void changeNick(String message) {
        ...
        if (!(isRegistered(newNick))) {
            try (Connection connection = DBConnection.getConnection()) {
                ...
                boolean isCreateFile = src.renameTo(dest);
                if (!isCreateFile) {
